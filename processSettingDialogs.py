...
class fillNADialog(QDialog):
    ...
    def addProcess(self):
        describe = {
            'FILL_VALUE': 'Fill NA with value (' + str(self.value) + ')',
            'FILL_MEAN': 'Fill NA with mean of column',
            'FILL_CLASS_MEAN': 'Fill NA with mean of column while belong to same class',
            'FILL_MEDIAN': 'Fill NA with median of column',
            'FILL_BAYESIAN': 'Fill NA with Bayesian method',
            'IGNORE': 'Ignore NA, Do Nothing',
            'DELETE': 'Delete rows with NA'
        }
